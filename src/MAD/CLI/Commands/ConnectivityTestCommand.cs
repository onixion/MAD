...
namespace MAD.CLICore
{
    class ConnectivityTestCommand : Command 
    {
            ...
            switch (_intensity)
            {
                ...
                case 5:
                    ...
                    if (String.IsNullOrEmpty(_ipString))
                    {
                        ...
                        return "Please give an additional IP Address via the -ip parameter";
                    }
                    else
        ...
        public static string Intensity1Check()
        {
            PingReply _reply;
            try
            {
        ...
        public static string Intensity2Check()
        {
			PingReply _reply1;
			PingReply _reply2;
			ushort _success = 0;
            ...
            try
            {
                ...
                {
                    Logger.Log("Connectivity check succseeded", Logger.MessageType.INFORM);
                    return "(2/2) worked! You seem to be connected!";
                }
                else
                {
                    Logger.Log("Connectivity check reports: " + _success.ToString() + "/2) worked..", Logger.MessageType.WARNING);
                    return "(" + _success.ToString() + "/2) worked.. You seem to have a problem.. " +
                        "\n Ping auf Google Primary DNS: " + _reply1.Status.ToString() +
                        "\n Ping auf Google Secondary DNS: " + _reply2.Status.ToString();
                }
            }
			catch (Exception ex)
            { 
		...
		public static string Intensity3Check()
		{
			PingReply _reply1;
			PingReply _reply2;
			PingReply _reply3; 
			...
			try
			{
				_reply1 = _ping.Send(IPAddress.Parse("8.8.8.8"));
				_reply2 = _ping.Send(IPAddress.Parse("8.8.4.4"));
                _reply3 = _ping.Send(IPAddress.Parse("194.232.104.140"));
                ...
                {
                    Logger.Log("Connectivity check succseeded", Logger.MessageType.INFORM);
                    return "(3/3) worked! You seem to be connected!";
                }
                else
                {
                    Logger.Log("Connectivity check reports: " + _success.ToString() + "/3) worked..", Logger.MessageType.WARNING);
                    return "(" + _success.ToString() + "/3) worked.. You seem to have a problem.. " +
                        "\n Ping on Google Primary DNS: " + _reply1.Status.ToString() +
                        "\n Ping on Google Secondary DNS: " + _reply2.Status.ToString() +
                        "\n Ping on orf.at: " + _reply3.Status.ToString();
                }
			}
			catch (Exception ex)
			{ 
		...
		public static string Intensity4Check()
		{
			...
			try
			{
                ...
                {
                    Logger.Log("Connectivity check succseeded", Logger.MessageType.INFORM);
                    return "(4/4) worked! You seem to be connected!";
                }
                else
                {
                    Logger.Log("Connectivity check reports: " + _success.ToString() + "/4) worked..", Logger.MessageType.WARNING);
                    return "(" + _success.ToString() + "/4) worked.. You seem to have a problem.. " +
                        "\n Ping on Google Primary DNS: " + _reply1.Status.ToString() +
                        "\n Ping on Google Secondary DNS: " + _reply2.Status.ToString() +
                        "\n Ping on orf.at: " + _reply3.Status.ToString() +
                        "\n HTTP-Request on google.com: " + _httpExceptionMessage;
                }
			}
			catch (Exception ex)
			{ 
                Logger.Log("Connectivity Check: An Error occured: " + ex.Data.ToString(), Logger.MessageType.ERROR);
				return "A Error with the Ping occured! \n ErrorStatus: " + ex.Data.ToString(); 
			}
		}

		public static string Intensity5Check(string _ipString)
		{
			string _httpExceptionMessage = "";
			...
			try
			{
                ...
                _reply3 = _ping.Send(IPAddress.Parse("194.232.104.140"));			//usa.gov -> because .. american government
                ...
                {
                    Logger.Log("Connectivity check succseeded", Logger.MessageType.INFORM);
                    return "(5/5) worked! You seem to be connected!";
                }
                else
                {
                    Logger.Log("Connectivity check reports: " + _success.ToString() + "/2) worked..", Logger.MessageType.WARNING);
                    return "(" + _success.ToString() + "/5) worked.. You seem to have a problem.. " +
                        "\n Ping on Google Primary DNS: " + _reply1.Status.ToString() +
                        "\n Ping on GOogle Secondary DNS: " + _reply2.Status.ToString() +
                        "\n Ping on orf.at: " + _reply3.Status.ToString() +
                        "\n Custom-Ping: " + _reply4.Status.ToString() +
                        "\n HTTP-Request on google.com: " + _httpExceptionMessage;
                }
			}
			catch (Exception ex)
